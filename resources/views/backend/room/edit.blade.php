    ...
    <div class="container-fluid">
        ...
        <div class="col-lg-12">
            <div class="row">
                <!-- FORM Panel -->
                <div class="col-md-4">
                    <form method="post" action="{{route("dashboard.room.update",$room->id)}}" id="manage-deductions">
                        @csrf
                        @method('put')
                        <div class="card">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="hall" id="exampleFormControlSelect1">
                                        @foreach($halls as $hall)
                                            <option {{$hall->id==$room->hall_id?'selected':''}} value="{{$hall->id}}">{{$hall->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Room Name</label>
                                    <textarea name="room" id="" cols="30" rows="2" class="form-control" required="">{{$room->name}}</textarea>
                                </div>
                            ...
                            <div class="card-footer">
                                <div class="row">
                                    <div class="col-md-12">
                                        <button class="btn btn-sm-5 btn-primary "> Update Room</button>
